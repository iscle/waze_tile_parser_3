...
fun <T> Iterable<T>.chunkedExact(size: Int): List<List<T>> {
    val chunks = chunked(size)
    if (chunks.isNotEmpty() && chunks[chunks.lastIndex].size != size) {
...
class Tile2 {
    ...
    class Builder {
        // __fill_line
        private var lineData: List<ByteArray>? = null
        private var lineSummary: LineSummary? = null
...
class TileParser {
    ...
    private fun parseTile(byteBuffer: ByteBuffer, id: Int, line: Int): RawTile {
        ...
        return parseDecompressedTile(decompressedBytes, id, line)
    }

    private fun parseDecompressedTile(byteArray: ByteArray, id: Int, line: Int): RawTile {
        ...
        val items = List(itemCount) { i ->
            ...
            val padSize = previousSizeWithPad - previousSize

            val currentPosition = byteBuffer.position()

            // advance the buffer position
            byteBuffer.position(currentPosition + padSize + size)

            // return the item data as a sublist to avoid copying the data
            byteList.subList(currentPosition + padSize, currentPosition + padSize + size)
        }

        val builder = Tile2.Builder()

        parseLine(builder, items)
        parsePoint(builder, items)
        parseShape(builder, items)
        parseString(builder, items)
        parseRoute(builder, items)
        parseStreet(builder, items)
        parsePolygon(builder, items)
        parseSpeed(builder, items)
        parseRange(builder, items)
        parseAlert(builder, items)
        parseTile(builder, items)
        parseMetadata(builder, items)
        parseVenue(builder, items)
        parseLineExt(builder, items)
        parsePolygonExt(builder, items)
        parseBeacon(builder, items)
        parseProto(builder, items)

        return RawTile(id, line, items)
    }

    private fun parseLine(builder: Tile2.Builder, items: List<List<Byte>>) {
        // split line data in arrays of 8 bytes
        val data = items[9].chunkedExact(8)
        val summary = items[10].chunkedExact(62).let {
            if (it.size != 1) throw IllegalArgumentException("Invalid line summary")
            it[0]
        }
        val broken = items[11].chunkedExact(2)
        val roundabout = items[12].chunkedExact(2)
    }

    private fun parsePoint(builder: Tile2.Builder, items: List<List<Byte>>) {
        val data = items[13].chunkedExact(4)
        val id = items[14].chunkedExact(4)
    }

    private fun parseShape(builder: Tile2.Builder, items: List<List<Byte>>) {
        val data = items[8].chunkedExact(4)
    }

    private fun parseString(builder: Tile2.Builder, items: List<List<Byte>>) {
        val string1 = items[0].chunkedExact(1)
        val string2 = items[1].chunkedExact(1)
        val string3 = items[2].chunkedExact(1)
        val string4 = items[3].chunkedExact(1)
        val string5 = items[4].chunkedExact(1)
        val string6 = items[5].chunkedExact(1)
        val string7 = items[6].chunkedExact(1)
        val string8 = items[7].chunkedExact(1)
    }

    private fun parseRoute(builder: Tile2.Builder, items: List<List<Byte>>) {
        val data = items[15].chunkedExact(4)
    }

    private fun parseStreet(builder: Tile2.Builder, items: List<List<Byte>>) {
        val name = items[16].chunkedExact(10)
        val city = items[17].chunkedExact(4)
        val id = items[37].chunkedExact(4)
    }

    private fun parsePolygon(builder: Tile2.Builder, items: List<List<Byte>>) {
        val head = items[18].chunkedExact(16)
        val point = items[19].chunkedExact(2)
    }

    private fun parseSpeed(builder: Tile2.Builder, items: List<List<Byte>>) {
        val avg = items[21].chunkedExact(2)
        val lineRef = items[20].chunkedExact(4)
        val index = items[22].chunkedExact(4)
        val index2 = items[23].chunkedExact(2)
    }

    private fun parseRange(builder: Tile2.Builder, items: List<List<Byte>>) {
        val range = items[24].chunkedExact(6)
    }

    private fun parseAlert(builder: Tile2.Builder, items: List<List<Byte>>) {
        val data = items[25].chunkedExact(16)
    }

    private fun parseTile(builder: Tile2.Builder, items: List<List<Byte>>) {
        val squareData = items[26].chunkedExact(12).let {
            if (it.size != 1) throw IllegalArgumentException("Invalid square data")
            it
        }
    }

    private fun parseMetadata(builder: Tile2.Builder, items: List<List<Byte>>) {
        val attributes = items[27].chunkedExact(8)
    }

    private fun parseVenue(builder: Tile2.Builder, items: List<List<Byte>>) {
        if (items.size < 29) return

        val head = items[28].chunkedExact(16)
        val venueId = items[29].chunkedExact(2)

        if (head.size != venueId.size) throw IllegalArgumentException("Invalid venue data")
    }

    private fun parseLineExt(builder: Tile2.Builder, items: List<List<Byte>>) {
        if (items.size < 31) return

        val type = items[30].chunkedExact(1)
        val id = items[31].chunkedExact(4)
        val levelByLine = items[32].chunkedExact(1)
        val levels = items[33].chunkedExact(1)
        val speedMax = items[34].chunkedExact(2)
        val attributes = items[36].chunkedExact(4)
        val laneType = items[43].chunkedExact(1)
        val newType = items[44].chunkedExact(1)
    }

    private fun parsePolygonExt(builder: Tile2.Builder, items: List<List<Byte>>) {
        if (items.size < 36) return

        val polygonEx = items[35].chunkedExact(4)
    }

    private fun parseBeacon(builder: Tile2.Builder, items: List<List<Byte>>) {
        if (items.size < 39) return

        val pos = items[38].chunkedExact(4)
        val id = items[39].chunkedExact(16)

        if (items.size < 41) return

        val exPos = items[40].chunkedExact(4)
        val exId = items[41].chunkedExact(16)
        val exMask = items[42].chunkedExact(1)
    }

    private fun parseProto(builder: Tile2.Builder, items: List<List<Byte>>) {
        if (items.size < 46) return

        val protoData = items[45].chunkedExact(1)
    }
}
